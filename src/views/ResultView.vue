...
export default {
  ...
  mounted () {
    let players = ['rock', 'paper', 'scissors']
    if (this.$route.query.type === 'a') players = ['rock', 'paper', 'scissors', 'lizard', 'spock']
    players.splice(players.indexOf(this.$route.params.char), 1)

    setTimeout(() => {
      this.house = players[Math.floor(Math.random() * players.length)]

      const player = this.$route.params.char

      if (player === 'paper' && (this.house === 'rock' || this.house === 'spock')) this.winner = player
      else if (player === 'rock' && (this.house === 'lizard' || this.house === 'scissors')) this.winner = player
      else if (player === 'scissors' && (this.house === 'paper' || this.house === 'lizard')) this.winner = player
      else if (player === 'lizard' && (this.house === 'paper' || this.house === 'spock')) this.winner = player
      else if (player === 'spock' && (this.house === 'rock' || this.house === 'scissors')) this.winner = player
      else this.winner = this.house

      if (this.winner === player) {
        this.$store.state.wins++
        localStorage.setItem('score', `${this.$store.state.wins}`)
      }
    }, 750)
  }
}
...
.result {
  ...
  &__head {
    ...
    justify-content: space-between;
    border: .2em solid variables.$header-outline;
  ...
  &__display {
    ...
    &__player {
      ...
      &.win {
        box-shadow: 0 0 0 6rem rgba(white, .04),
                    0 0 0 10rem rgba(white, .04),
                    0 0 0 14rem rgba(white, .04);
      }
      ...
      &__icon {
        ...
        justify-content: center;
        background-color: variables.$text;
        box-shadow: inset 0 .6rem 0 0 hsla(229, 25%, 31%, .2);
        ...
        &.paper {
          &::before, &::after {
            background-image: variables.$paper;
          }
        }

        &.scissors {
          &::before, &::after {
            background-image: variables.$scissors;
          }
        }

        &.rock {
          &::before, &::after {
            background-image: variables.$rock;
          }
        }

        &.lizard {
          &::before, &::after {
            background-image: variables.$lizard;
          }
        }

        &.spock {
          &::before, &::after {
            background-image: variables.$spock;
          }
        }
      }
    }
    ...
    &__replay {
      ...
      flex-direction: column;
      justify-content: center;
      transition: width 600ms ease-in-out, opacity 1200ms ease-in-out;
  ...
  &__foot {
    ...
    &__rules {
      ...
      border: .125rem solid variables.$header-outline;
